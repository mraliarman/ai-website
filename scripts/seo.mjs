...
function organization() {
	return {'@type':'Organization','@id':`${abs('/')}#organization`,'name':siteName,'url':abs('/'),'logo':{'@type':'ImageObject','url':abs('/assets/main.css')}};
}
function website() {
...
for (const file of files) {
	...
	if (!isHome) {
		graph.push({'@type':'BreadcrumbList','@id':`${canonical}#breadcrumb`,'itemListElement':[{'@type':'ListItem','position':1,'name':'خانه','item':abs('/')},{'@type':'ListItem','position':2,'name':title,'item':canonical}]});
	}
	if (pageType === 'Article') {
	...
	if (isPromptIndex) {
		graph.push({'@type':'CollectionPage','@id':`${canonical}#collection`,'url':canonical,'name':title,'description':description,'inLanguage':'fa-IR','isPartOf':{'@id':`${abs('/')}#website`}});
	}
	...
	html = html.replace(/<meta\s+name="robots"[\s\S]*?<\/script>/i, '');
