...
async function main() {
    ...
    for (const item of generated) {
        ...
        const body = `<article class="site-shell py-16 sm:py-20"><div class="text-xs text-smoke">خانه / ${item[1]}</div><header class="mt-10 border-b border-graphite pb-10 sm:mt-12"><span class="font-mono text-xs text-smoke">${String(item[0]).toUpperCase()}</span><h1 class="mt-5 max-w-5xl text-[34px] font-normal leading-[1.3] sm:text-[44px]">${item[1]}</h1><p class="mt-5 max-w-3xl text-base text-smoke sm:text-lg">${item[3]}</p></header>${toc ? `<section class="toc mt-8 p-4 sm:p-5" aria-labelledby="toc-title"><div class="mb-3 flex items-center justify-between gap-4"><h2 id="toc-title" class="text-lg font-normal text-chalk">فهرست مطالب</h2><span class="font-mono text-xs text-smoke">${[...item.markdown.matchAll(/^#{2,3}\s+/gm)].length} بخش</span></div><nav class="grid gap-0">${toc}</nav></section>` : ''}<div class="article-layout mt-12"><div class="article-content"><div class="prose-llm">${html}</div></div></div></article>`;
